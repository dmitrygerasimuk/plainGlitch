...
void LoadImage(char* imagefile) {
     ...
     unsigned char* my_image_id = stbi_load(imagefile, &width, &height, &bpp, 3 );
        if (my_image_id == NULL)
        {
 ...
 mylog.AddLog("[info] loaded %s into memory \n",imagefile);
...
if(bpp == 3)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, my_image_id);
else if(bpp == 4)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, my_image_id);
...
*/
 ...
 // ImFont* font = io.Fonts->AddFontFromFileTTF("./keyrusMedium.ttf", 18.0f, NULL, io.Fonts->GetGlyphRangesCyrillic());
  ...
  IM_ASSERT(font != NULL);
    ...
    bool show_proxygen_window = false;
...
//---------
		...
		ImGui::Text("%6d/%-6d %6d lines  | %s | %s | %s | %s %s", cpos.mLine + 1, cpos.mColumn + 1, editor.GetTotalLines(),
			...
			if (changesMade) ImGui::Text("Changed");
...
static float f = 0.0f;
           ...
           // ImGui::ColorEdit3("clear color", (float*)&clear_color);
            ...
            snprintf(exepath,512,"./ffmpeg -y -itsoffset -1 -ss `./ffmpeg -i \"%s\" 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}' | cut -d',' -f1 ` -i \"%s\" -vframes 1 -filter:v scale=\"280:-1\"  $TMPDIR/thumb.jpg",outPath,outPath);
...
ImGui::LabelText("Timecode start", ""); 
      ...
      ImGui::PopItemWidth(); 
            ...
            ImGui::NextColumn();
             ImGui::InputText("Watermark", outWaterText,64);
            ImGui::InputText("Watermark Color", outWaterColor,12);
           // ImGui::InputFloat("Watermark Opacity",&outWaterOpac,0.1f,0,1.0f);  
             ImGui::SliderFloat("Watermark Opacity", &outWaterOpac, 0.0f, 1.0f, "opacity = %.1f");    
             
              ImGui::InputInt("Watermark Size", &outWaterSize);
           // ImGui::InputInt("Watermark X", &outWaterX);
             ImGui::SliderInt("Watermark X", &outWaterX, 0, outWi);     
                      ImGui::SliderInt("Watermark Y", &outWaterY, 0, outHe);   
...
*/
   ...
   else 
   {
            ...
            remove(testthumbpath);
                    ...
                    snprintf(testframepath,1024,"./ffmpeg  -nostdin  -loglevel info -y -ss `./ffmpeg -i \"%s\" 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}' | cut -d',' -f1 ` -i \"%s\" -vframes 1 -vf \"scale=%d:%d, drawtext=fontfile=%s: text='%s': fontcolor=%s@%.1f: fontsize=%d: x=%d: y=%d,drawtext=fontfile=vcr.ttf: timecode='%d\\:%d\\:%d\\:%d': r=25: fontcolor=0xFFFFFF: fontsize=48: x=480: y=650,drawtext=fontfile=\"vcr.ttf\": text='': fontcolor=0xFFFFFF@0.5: fontsize=512: x=600: y=360\"  \"%stestframe.jpg\"",outPath,outPath,outWi,outHe,outFont,outWaterText,outWaterColor,outWaterOpac,outWaterSize,outWaterX,outWaterY,timecode[0],timecode[1],timecode[2],timecode[3],folder);
                    printf("\n%s\n",testframepath);
                    mylog.AddLog("[info] exec: %s\n",testframepath);
...
//ImGui::Text("Asdad\n");
 ...
 static char buf2[64] = "batman.jpg"; ImGui::InputText("a", buf2, 64);  
   if (ImGui::Button("Load")) { LoadImage(buf2);}
